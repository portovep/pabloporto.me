...
export default function BookCard({ book, coverPath }: BookCardProps) {
    ...
    return (
        <Link href={bookUrl} target="_blank" rel="noopener noreferrer">
            <Card
                className="group h-full transition-all hover:shadow-lg border-border bg-card"
                data-testid="book-card">
                <CardContent className="p-0">
                    <div className="relative aspect-[2/3] w-full overflow-hidden bg-muted flex items-center justify-center">
                        {coverSrc && (
                            <Image
                                src={coverSrc}
                                alt={book.title}
